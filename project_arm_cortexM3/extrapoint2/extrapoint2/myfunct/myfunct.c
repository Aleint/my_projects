...
static int numMove=0,overL=0,overR=0,overU=0,overD=0;//step over the token if there is a wall
...
displayMoves(1);
	switch (cmd) {
        ...
        case SEL:
            ...
            board[previousTokenPos[player].y][previousTokenPos[player].x] = EMPTY_CELL;
						board[lastTokenPos[player].y][lastTokenPos[player].x] = player ? PLAYER_2_CELL: PLAYER_1_CELL;  
								...
								moves[numMove].dir=0;
								moves[numMove].x=lastTokenPos[player].x/2;
								moves[numMove].y=lastTokenPos[player].y/2;
								numMove++;
           endTurn=1;
        ...
        case L:
					if(overU && board[lastTokenPos[player].y-2][lastTokenPos[player].x-1]==EMPTY_CELL && board[lastTokenPos[player].y-2][lastTokenPos[player].x-2]==EMPTY_CELL){
					...
					if(overD && board[lastTokenPos[player].y+2][lastTokenPos[player].x-1]==EMPTY_CELL &&board[lastTokenPos[player].y+2][lastTokenPos[player].x-2]==EMPTY_CELL){
						drawToken(tokenx,tokeny,Blue2);//delete token
							lastTokenPos[player].y+=2;
							lastTokenPos[player].x-=2;
							tokeny=10+((lastTokenPos[player].y/2)*30)+15;
        ...
        case R:
            ...
            break;
        case U:
					if(overL && board[lastTokenPos[player].y-1][lastTokenPos[player].x-2]==EMPTY_CELL &&board[lastTokenPos[player].y-2][lastTokenPos[player].x-2]==EMPTY_CELL){
							drawToken(tokenx,tokeny,Blue2);//delete token
							lastTokenPos[player].y-=2;
							lastTokenPos[player].x-=2;
							tokeny=10+((lastTokenPos[player].y/2)*30)+15;
					...
					if(overR && board[lastTokenPos[player].y-1][lastTokenPos[player].x+2]==EMPTY_CELL && board[lastTokenPos[player].y-2][lastTokenPos[player].x+2]==EMPTY_CELL){
							drawToken(tokenx,tokeny,Blue2);//delete token
								lastTokenPos[player].y-=2;
								lastTokenPos[player].x+=2;
							tokeny=10+((lastTokenPos[player].y/2)*30)+15;
					...
					for (i=wallPos.x-1;i<wallPos.x+2;i++){
							if(board[wallPos.y][i]!=0){
								GUI_Text(box1X, boxY-20,(uint8_t *)"wall overlap", Black, player? Red:White);
									delay_s(1,box1X, boxY-20,"wall overlap");							
								overlap=1;
								break;
							}
						}
					if(!overlap){
						...
						for (i=wallPos.x-1;i<wallPos.x+2;i++)									
							updateBoard(wallPos.y,i,WALL_CELL_H);
								...
								moves[numMove].dir=1;
								moves[numMove].x=wallPos.x/2;
								moves[numMove].y=wallPos.y/2;
								numMove++;
								}
								else{
									GUI_Text(box1X, boxY-20,(uint8_t *)"The wall traps the token.", Black, player? Red:White);
									delay_s(1,box1X, boxY-20,"The wall traps the token.");								
										for (i=wallPos.x-1;i<wallPos.x+2;i++)									
...
//case L:
            ...
            if(lastTokenPos[player].x-2>=0 && board[lastTokenPos[player].y][lastTokenPos[player].x-1]==EMPTY_CELL ){//no walls						
							...
							//over
										...
										x=lastTokenPos[player].x-2;		
										x=10+((x/2)*30)+15;
									if(board[lastTokenPos[player].y-1][lastTokenPos[player].x-2]==EMPTY_CELL &&board[lastTokenPos[player].y-2][lastTokenPos[player].x-2]==EMPTY_CELL){
									y=lastTokenPos[player].y-2;
									y=10+((y/2)*30)+15;
										if(update){
											drawToken(x,y, Blue2);//delete previous possible position								
								}
										else{
										drawToken(x,y, Black);
										}
            ...
            if(lastTokenPos[player].y-2>=0 && board[lastTokenPos[player].y-1][lastTokenPos[player].x]==EMPTY_CELL){//no wall
							...
							drawToken(tokenx,y,Blue2);	
								...
								//over
										...
										y=lastTokenPos[player].y-2;		
										y=10+((y/2)*30)+15;
									if(board[lastTokenPos[player].y-2][lastTokenPos[player].x+1]==EMPTY_CELL &&board[lastTokenPos[player].y-2][lastTokenPos[player].x+2]==EMPTY_CELL){
...
volatile int stringx=0;
volatile int stringy=0;
volatile int stringc1=0;
volatile int stringc2=0;
...
void connectBoards(){
uint8_t *tempStr1 = (uint8_t*)"Connecting . . .";	
uint8_t *tempStr2 = (uint8_t*)"Connection failed.";	
uint16_t centerX =0;
uint16_t centerY =0;
uint8_t *startGameText ;
uint16_t textLength = 0;
uint16_t charWidth = 8;
...
while (*tempStr2 != 0)
    {
     ...
     centerY = (MAX_Y - 20) / 2;  // Assuming 20 pixels for the height of a line
		...
		LCD_Clear(Blue2);
    GUI_Text(centerX, centerY, (uint8_t*)"Connection failed. ", Black, Blue2);
		GUI_Text(centerX-20, centerY+20, (uint8_t*)" Press reset to restart", Black, Blue2);
		connect =1;
		disable_timer(1);
}
else{
while (*tempStr1 != 0)
    {
     ...
     centerY = (MAX_Y - 20) / 2;  // Assuming 20 pixels for the height of a line
		...
		LCD_Clear(Blue2);
		GUI_Text(centerX-20, centerY-10, (uint8_t*)"Press INT0 to connect", Black, Blue2);
    GUI_Text(centerX, centerY+10, (uint8_t*)"Connecting . . .", Black, Blue2);
...
switch(cmd){
	...
	case SEL:
		...
		//display second menu
			...
			if(mode==2)//2 boards ,try connection
			{connectBoards();
			}
			else{
			LCD_Clear(Blue2);
...
volatile int NPCplacewall=0;//quanti turni sono passati senza piazzare un muro
void NPC(){
int x=lastTokenPos[player].x;
int y=lastTokenPos[player].y;
int ok,randomMove=0;

		if(NPCplacewall<=2 ){
			NPCplacewall++;
			//NPC is player 2 priority to UP     
    if (isWithinBounds(x, y - 2) && board[y - 1][x] != WALL_CELL_H && board[y - 1][x] != WALL_CELL_V && board[y - 2][x] != -1 ) {	
         moveToken(U);
    }
            ...
            // Randomly choose left or right
             randomMove = rand() % 2;
            if (randomMove == 0) {
                if (isWithinBounds(x + 2, y) && board[y][x + 1] != WALL_CELL_H && board[y][x + 1] != WALL_CELL_V && board[y][x + 2] != -1) {
                    moveToken(R);
										ok=1;
                }
            }
		...
		if(walls[player]!=0){//place horizontal wall in front of token	
			dirWall=1;
		wallPos.y=lastTokenPos[0].y+1;
		
		if(isWithinBounds(lastTokenPos[0].x-1, wallPos.y)){
		wallPos.x=lastTokenPos[0].x-1;
	if(moveWall(SEL)){
		NPCplacewall=1;
		endTurn=1;
		return;
...
void transmitMove(){
					...
					CAN_TxMsg.data[0] = moves[numMove].playerID  ;
					CAN_TxMsg.data[1] = (CAN_TxMsg.data[1] & 0x0F) |(moves[numMove].playerMove	)	;	
					CAN_TxMsg.data[1] = (CAN_TxMsg.data[1] & 0xF0) |(moves[numMove].dir ) ;
					CAN_TxMsg.data[2] = moves[numMove].x;
					CAN_TxMsg.data[3] = moves[numMove].y;
					CAN_TxMsg.len = 4;
					CAN_TxMsg.id = 2;
					CAN_TxMsg.format = STANDARD_FORMAT;
					CAN_TxMsg.type = DATA_FRAME;
