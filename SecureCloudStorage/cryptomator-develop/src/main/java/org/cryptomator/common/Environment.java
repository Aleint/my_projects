...
public class Environment {
	...
	private Stream<Path> pathMerge(Stream<Path> str) {
		List<Path> isp = new ArrayList<>(str.toList());
		Path p;
		if (isp.size() >= 2) {
			p = Paths.get(isp.get(0).toString() + ":" + isp.get(1).toString());		}
		else {
			p = isp.get(0);
		}
	...
	public Stream<Path> getSettingsPath() {
		return pathMerge(getPaths("cryptomator.settingsPath"));
	}

	public Stream<Path> ipcSocketPath() {
		return pathMerge(getPaths("cryptomator.ipcSocketPath"));
	}
